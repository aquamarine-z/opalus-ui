...
Surface waits for the finite Web Animations attached to
`SurfaceDrawerContent`. The public Promise resolves and the NiceModal instance
is removed only after those animations settle. Surface does not use a fixed
`400ms`, `500ms`, or other JavaScript close timeout.
